const { DeleteCommissionCsService } = require("../../services/commissionsCs/DeleteCommissionCsService");
...
module.exports.DeleteCommissionCsController = async (req, res) => {
  try {
    const idComissao = req.params.id;
    const { publicIdImage } = req.body;
    const deleteCommissionCsService = await DeleteCommissionCsService(idComissao);

    if(deleteCommissionCsService) {
